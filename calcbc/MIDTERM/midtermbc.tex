...
\subsection*{H27 Volumes}

\subsubsection*{Disc method}
The disc method is used to find the volume of a solid of revolution by dividing the solid into discs perpendicular to the axis of revolution. The volume of each disc is calculated and then integrated over the interval:

$$ V = \pi \int_a^b [R(x)]^2 \: dx $$

where $R(x)$ is the radius of the disc at position $x$.

\example{Find the volume of the solid formed by revolving the region bounded by the curve $y = \sqrt{x}$ and $x = 4$ about the $x$-axis.}

\step{1}{Draw the graph.}

\begin{figure}[H]
\centering
\includegraphics[width=0.5\textwidth]{discmethod}
\end{figure}

\step{2}{Set up the integral using the disc method and solve.}

In this case, the radius is the function, but the radius is not always the function.

\begin{gather*}
V = \pi \int_0^4 (\sqrt{x})^2 \: dx \\
= \pi \int_0^4 x \: dx \\
= \pi \cdot \left[\frac{1}{2}x^2\right]_0^4 \\
= \boxed{8\pi}
\end{gather*}

\subsubsection*{Washer method}

The washer method is an extension of the disc method used when the solid of revolution has a hole in the center. The volume is calculated by subtracting the volume of the inner disc from the outer disc:

$$ V = \pi \int_a^b \left( [R(x)]^2 - [r(x)]^2 \right) \: dx $$

where $R(x)$ is the outer radius and $r(x)$ is the inner radius at position $x$.

\example{Find the volume of the solid formed by revolving the region bounded by the curves $y = \sqrt{x}$ and $x=4$ about the $y$-axis.}

\step{1}{Draw the graph.}

The graph is the same as the previous example, but this time the axis of rotation is the $y$-axis. The washer method is needed because the region does not touch the axis of rotation.

\step{2}{Write the equations in terms of $y$.}

$y = \sqrt{x}$, so $x = y^2$. The line $x = 4$ is 4 away from the $y$-axis, so the outer radius stays the same. 

\step{3}{Set up the integral using the washer method and solve.}

\begin{gather*}
V = \pi \int_0^2 [(4^2 - (y^2)^2] \: dy \\
= \pi \int_0^2 (16 - y^4) \: dy \\
= \pi \cdot \left[16y - \frac{1}{5}y^5 \right]_0^2 \\
= \boxed{\frac{128\pi}{5}}
\end{gather*}

\subsection*{H28 Cross sections}

The volume of a solid with known cross-sectional area can be found by integrating the area of the cross-sections over the interval:

$$ V = \int_a^b A(x) \: dx $$

where $A(x)$ is the area of the cross-section at position $x$ or

$$ V = \int_c^d A(y) \: dy $$

where $A(y)$ is the area of the cross-section at position $y$.

\example{The region formed by the line $x+2y = 2$, the $x$-axis, and the $y$-axis is the base for a solid. The cross-sections \textbf{perpendicular to the $x$-axis} are \textbf{equilateral triangles}, with one side of the triangle in the $xy$ plane. What is the volume of the solid?}

Because the base is perpendicular to the $x$-axis, integrate wrt $x$.

\step{1}{Draw the graph.}

\begin{figure}[H]
\centering
\includegraphics[width=0.5\textwidth]{crosssecs}
\end{figure}

\step{2}{Set up the equation for the area of one triangle.}

The area of an equilateral triangle is given by $$A = \frac{\sqrt{3}}{4}s^2$$ where $s$ is the length of one side. The length of each side is the function itself, so the area of each cross-section is
...
\begin{gather*}
V = \int_0^2 \frac{\sqrt{3}}{4} (2 - x)^2 \: dx \\
= \frac{\sqrt{3}}{4} \int_0^2 (2 - x)^2 \: dx \\
= \frac{\sqrt{3}}{4} \cdot \left[ \frac{(2 - x)^3}{-3} \right]_0^2 \\
= \boxed{\frac{2\sqrt{3}}{3}}
\end{gather*}

\subsection*{H29 Improper integrals}

An improper integral is an integral that has either an infinite limit of integration or an integrand that approaches infinity within the limits of integration.

To evaluate an improper integral:

\begin{enumerate}
\item Identify the type of improper integral (infinite limit or infinite integrand).
\item Rewrite the integral as a limit.
\item Evaluate the limit.
\end{enumerate}

\example{Evaluate the integral $\displaystyle \int_{-\infty}^{0} \frac{1}{(x-2)^3} \: dx$. Determine whether it converges or diverges.}

\step{1}{Rewrite the integral as a limit.}

$$\lim_{b \to -\infty} \int_b^0 \frac{1}{(x-2)^3} \: dx$$

\step{2}{Evaluate the limit and determine whether it diverges ($\infty$ or $-\infty$) or converges (number).}

\begin{gather*}
\lim_{b \to -\infty} \left[ \frac{1}{-2(x-2)^2} \right]_b^0 \\
= \lim_{b \to -\infty} \left( -\frac{1}{8} + \frac{1}{2(b-2)^2} \right) \\
= -\frac{1}{8} + 0 \\
\boxed{\text{converges to} \: -\frac{1}{8}}
\end{gather*}

\example{Evaluate the integral $\displaystyle \int_{-\infty}^{\infty} e^{2x} \: dx$. Determine whether it converges or diverges.}

\step{1}{Rewrite the integral as a limit.}
$$\lim_{a \to -\infty} \int_a^0 e^{2x} \: dx + \lim_{b \to \infty} \int_0^b e^{2x} \: dx$$

\step{2}{Evaluate the limit.}
\begin{gather*}
\lim_{a \to -\infty} \left[ \frac{1}{2} e^{2x} \right]_a^0 + \lim_{b \to \infty} \left[ \frac{1}{2} e^{2x} \right]_0^b \\
= \lim_{a \to -\infty} \left( \frac{1}{2} - \frac{1}{2} e^{2a} \right) + \lim_{b \to \infty} \left( \frac{1}{2} e^{2b} - \frac{1}{2} \right) \\
= \left( \frac{1}{2} - 0 \right) + \left( \infty - \frac{1}{2} \right) \\
\boxed{\text{diverges}}
\end{gather*}

This limit diverges because $\displaystyle \lim_{b \to \infty} \frac{1}{2} e^{2b} = \infty$. If one of the terms is $\pm \infty$, then the entire limit diverges.

\example{Evaluate the integral $\displaystyle \int_0^4 \frac{1}{\sqrt{4-r}} \: dr$. Determine whether it converges or diverges.}

\step{1}{Rewrite the integral as a limit.}

$$\lim_{b \to 4^-} \int_0^b \frac{1}{\sqrt{4-r}} \: dr$$

The limit approaches 4 \textbf{from the left} because the interval approaches 4 from the left (0 to 4 as opposed to 4 to a certain number).

\step{2}{Evaluate the limit.}

\begin{gather*}
\lim_{b \to 4^-} \left[ -2\sqrt{4-r} \right]_0^b \\
= \lim_{b \to 4^-} \left( -2\sqrt{4-b} + 4 \right) \\
= -2(0) + 4 \\
\boxed{\text{converges to} \: 4}
\end{gather*}

\subsection*{H31 Arc length (H30 review, skipped)}
